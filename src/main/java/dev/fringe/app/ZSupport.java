...
public class ZSupport {
    ...
    public List<Strategy> getTestStrategies(String name, BarSeries series, Integer start, Integer barCount, Integer step) {
		if(start == null) {
			start = 0;
		}
		if(barCount == null) {
	...
	public List<Strategy> getTestStrategies(String name, BarSeries series) {
		return this.getTestStrategies(name, series, null, null, null);
	}

	
	public BarSeries getTestBarSeries(String barName) {
		BarSeries series = getTestBars(barName);
...
//	    	System.out.println("Volume: " + series.getBar(i).getVolume() + "\t" + "Open price: "+ series.getBar(i).getOpenPrice() + "\t" + "Close price: " + series.getBar(i).getClosePrice() + "\t" +  series.getBar(i).getDateName() );
	...
	private BarSeries getTestBars(String barName) {
